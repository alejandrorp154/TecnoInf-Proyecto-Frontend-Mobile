...
import { Chatroom, ChatService, imgFile } from '../../servicios/chat.service';
...
export class ChatPage implements OnInit {
  ...
  chatWith(amigo: Usuario) {
    console.log(amigo);
    if (this.chatrooms.some(c => c.uids == [this.currentUser.uid, amigo.uid])) {
  ...
  getFriendImage(chatroom: Chatroom) {
    if (chatroom.uids.length > 2) {
      ...
      return this.friends.find(f => f.uid == chatroom.uids.find(f => f != this.currentUser.uid)).imgurl;
    }
  }
  ...
  getChatroomName(chatroom: Chatroom) {
    if (chatroom.uids.length > 2) {
      ...
      return this.friends.find(f => f.uid == chatroom.uids.find(f => f != this.currentUser.uid)).nombre + ' ' +
      this.friends.find(f => f.uid == chatroom.uids.find(f => f != this.currentUser.uid)).apellido;
    }
  }
